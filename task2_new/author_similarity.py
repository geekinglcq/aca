...
def calculate_similarity(list_1,list_2):
    ...
    #    if v in list_1:
    #        num += 1
    num = (len(set(list_1)&set(list_2))/len(set(list_2)))*math.exp(1/len(list_1)**4)
    return num
...
def similarity_by_indx(author_train,author_vali,author_interest):
...
def similarity_by_press(author_train,author_vali,author_interest):
...
def predict_by_press_similarity(author_train,author_vali,t_author_press,p_author_press,author_interest):
    ...
    for author in author_vali:
        ...
        flag += 1
        press_list = []
        press_list.extend(t_author_press)
        #for indx in author_indx_citeindx[author].keys():
        #    indx_list.append(str(indx))
        #    for k,v in indx_neighbors[str(indx)].items():
        #        indx_list.extend(v)
