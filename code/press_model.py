...
def predict_test_interest(author_test,author_interest,press_interest,t_author_press,p_author_press):
    ...
    with codecs.open("./raw_data/p_author_press_ex_cite.json","r","utf-8") as fid:
        cite_author_press = json.load(fid)
    with codecs.open("./raw_data/p_author_press_ex_coop.json","r","utf-8") as fid:
        coop_author_press = json.load(fid)
    ...
    for vali_m in range(1):
        ...
        for author in author_test:
            press_list = p_author_press[author]

            if len(p_author_press[author]) <= 10:
                press_list.extend(shuffle_list(cite_author_press[author],30))

            p_interest,p_interest_score = find_interest_by_count(press_interest,press_list,press_score,all_press_count)
...
def predict_vali_interest(author_vali,author_interest,press_interest,t_author_press):
    ...
    for vali_m in range(1):
        ...
        for author in author_vali:
            press_list = t_author_press[author]
            #if len(t_author_press[author]) > 10:
            #    p_author_interest.setdefault(author,[])
            #    flag +=1
            #    continue
            if len(t_author_press[author]) <= 10:
            #press_list.extend(shuffle_list(coop_author_press[author],10))
                press_list.extend(shuffle_list(cite_author_press[author],30))
